...
// MigrateOption configura el runner de migraciones.
...
func WithAdvisoryLock(timeout time.Duration) MigrateOption {
...
// MigrateUp ejecuta migraciones SQL embebidas contra la DB de GORM.
func MigrateUp(db *gorm.DB, sqlFiles fs.FS, subdir string, opts ...MigrateOption) error {
	if subdir == "" {
		subdir = "."
	}
	...
	if cfg.advisoryLock {
		unlock, lockErr := acquireAdvisoryLock(context.Background(), sqlDB, cfg.migrationsTable, cfg.lockTimeout)
		if lockErr != nil {
...
// MigrateUpFromDir ejecuta migraciones desde un directorio del filesystem (no embed.FS).
func MigrateUpFromDir(db *gorm.DB, dir string, opts ...MigrateOption) error {
	cfg := &migrateConfig{}
	for _, opt := range opts {
		opt(cfg)
	}
...
func acquireAdvisoryLock(ctx context.Context, sqlDB *sql.DB, name string, timeout time.Duration) (func(), error) {
	if timeout <= 0 {
